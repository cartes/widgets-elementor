...
class ELPL_Post_Layout_Widget extends \Elementor\Widget_Base
{
	...
	protected function register_controls()
	{
		...
		$this->start_controls_section(
			'query_section',
			array(
				'label' => esc_html__('Query', 'elementor-post-layout'),
				'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
			)
		);
		...
		$this->add_control(
			'date_range',
			array(
				'label' => esc_html__('Corte de Fechas', 'elementor-post-layout'),
				'type' => \Elementor\Controls_Manager::SELECT,
				'options' => array(
					''           => esc_html__('Sin filtro (todos)', 'elementor-post-layout'),
					'yesterday'  => esc_html__('Hasta ayer (excluye hoy)', 'elementor-post-layout'),
					'last_week'  => esc_html__('Hasta fin de semana pasada', 'elementor-post-layout'),
					'last_month' => esc_html__('Hasta fin del mes anterior', 'elementor-post-layout'),
				),
				'default' => '',
				'label_block' => true,
				'description' => esc_html__('Muestra los posts más recientes publicados hasta la fecha seleccionada. Siempre habrá contenido visible.', 'elementor-post-layout'),
			)
		);
	...
	/**
	 * Build a WP_Query date_query array from a date range key.
	 * Uses only an upper bound (before) so there are always posts to show.
	 *
	 * @param string $date_range  'yesterday' | 'last_week' | 'last_month' | ''
	 * @return array
	 */
	private function build_date_query($date_range)
	{
		switch ($date_range) {
			case 'yesterday':
				// Posts published up to and including yesterday (excludes today)
				return array(
					array(
			...
			case 'last_week':
				// Posts published up to and including the last day of last week (Sunday)
				return array(
					array(
			...
			case 'last_month':
				// Posts published up to and including the last day of last month
				return array(
					array(
						'before'    => date('Y-m-d 23:59:59', strtotime('last day of last month')),
						'inclusive' => true,
					),
				);
	...
	protected function render()
	{
		...
		// Google Ads code is output directly as it may contain <script> tags.
		// Elementor sanitizes it on save depending on the editing user's capabilities.
		...
		if (!empty($date_query)) {
			$mobile_args['date_query'] = $date_query;
		}
		...
		if (!empty($date_query)) {
			$more_check_args['date_query'] = $date_query;
		}
