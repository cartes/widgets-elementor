...
final class Elementor_Post_Layout
{
	...
	public function ajax_load_more_posts()
	{
		...
		$show_title = sanitize_key(wp_unslash($_POST['show_title'] ?? 'yes'));
		$title_tag = sanitize_key(wp_unslash($_POST['title_tag'] ?? 'h3'));
		$excerpt_length = absint($_POST['excerpt_length'] ?? 25);
		$show_read_more = sanitize_key(wp_unslash($_POST['show_read_more'] ?? 'yes'));
		$read_more_text = sanitize_text_field(wp_unslash($_POST['read_more_text'] ?? 'Leer más »'));
		$image_size = sanitize_text_field(wp_unslash($_POST['image_size'] ?? 'medium'));
			...
			while ($query->have_posts()) {
				...
				if ('elpl_noticias_generales' === $widget) {
					...
					<article class="elpl-archive-post elpl-side-post">
						<div class="elpl-archive-date-mobile">
							...
							<div class="elpl-archive-header">
								<?php if (!empty($meta_data)): ?>
									<div class="elpl-archive-meta elpl-archive-meta-mobile">
										<?php foreach ($meta_data as $meta_key): ?>
								...
								<?php if ($show_title === 'yes'): ?>
									<<?php echo esc_attr($title_tag); ?> class="elpl-archive-title">
